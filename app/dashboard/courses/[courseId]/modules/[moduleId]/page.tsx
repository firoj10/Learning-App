...
const Module = async ({ params }: PageProps) => {
  ...
  const moduleDoc: any = await getModule(moduleId);

  // ✅ IMPORTANT: make it a plain JSON-safe object for Client Components
  const module = JSON.parse(JSON.stringify(moduleDoc));
    ...
    <>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-16">
          ...
          <div>
            <div className="flex items-center gap-x-2">{/* empty */}</div>
          </div>
        </div>
      </div>
    </>
